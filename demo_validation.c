/**
 * @file demo_validation.c
 * @brief Demonstration of the new command validation utilities
 * 
...
int main() {
    ...
    //char* test_argv_empty[] = {"test_cmd"};
    //char* test_argv_valid[] = {"test_cmd", "arg1", "arg2"};
    ...
    // Test 4: Validate session IDs
    printf("\nTest 4: Session ID validation\n");
    unsigned int session_id;
    if (validate_session_id("123", &session_id)) {
        printf("  ✓ Valid session ID '123' -> %u\n", session_id);
    }
    
    if (!validate_session_id("-1", NULL)) {
        printf("  ✓ Invalid negative session ID '-1' correctly rejected\n");
    }
