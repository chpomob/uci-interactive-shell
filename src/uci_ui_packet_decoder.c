...
void ui_analyze_uci_packet(unsigned char* packet, size_t packet_len) {
    ...
    if (payload_len_field > 0) {
        ...
        if (get_mt(header) == RESPONSE && get_gid(header) == CORE) {
            switch(opcode) {
                ...
                case CORE_QUERY_UWBS_TIMESTAMP:
                    ...
                    break;
                case CORE_GET_STATE:
                    ui_decode_core_get_state_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_SET_ACTIVE:
                    ui_decode_core_set_active_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_SET_READY:
                    ui_decode_core_set_ready_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_DEVICE_READY:
                    ui_decode_core_device_ready_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_GET_CAPS:
                    ui_decode_core_get_caps_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_SET_POWER:
                    ui_decode_core_set_power_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_GET_POWER:
                    ui_decode_core_get_power_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_DEVICE_ON:
                    ui_decode_core_device_on_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_DEVICE_OFF:
                    ui_decode_core_device_off_rsp(payload_ptr, payload_len_int);
                    break;
                case CORE_DEVICE_SUSPEND_CMD:
                    ui_decode_core_device_suspend_cmd_rsp(payload_ptr, payload_len_int);
                    break;
                default:
                    if (ui_color_enabled) {
            ...
            switch(opcode) {
                case ANDROID_FIRA_RANGE_DIAGNOSTICS_NTF:
                    ui_decode_android_range_diagnostics_ntf(payload_ptr, payload_len_int);
                    break;
                default:
                    if (ui_color_enabled) {
        ...
        } else if (get_mt(header) == NOTIFICATION && get_gid(header) == RANGE_DATA_NTF_GID) {
            switch(opcode) {
                case RANGE_DATA_NTF_OID:
                    ui_decode_range_data_ntf(payload_ptr, payload_len_int);
                    break;
                default:
                    if (ui_color_enabled) {
                        printf("  %s%sNo specific decoder for RANGE_DATA_NOTIFICATION opcode 0x%02X%s\n", 
                               ANSI_COLOR_BRIGHT_BLACK, ANSI_BOLD, opcode, ANSI_RESET);
                    } else {
                        printf("  No specific decoder for RANGE_DATA_NOTIFICATION opcode 0x%02X\n", opcode);
                    }
                    break;
            }
        } else {
